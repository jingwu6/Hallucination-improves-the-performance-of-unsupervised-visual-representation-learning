...
def train(train_loader, model, criterion, optimizer, epoch, cfg, logger, writer):
    ...
    for idx, (images, _) in enumerate(train_loader):
        ...
        # compute output
        output, target, cos_loss = model(im_q=images[0], im_k=images[1])
        # loss = criterion(output, target) + 0.1*cos_loss if cos_loss != 0 else criterion(output, target)
        loss = criterion(output, target)
        # print('ck', loss, cos_loss)
        loss += cos_loss
...
def main_worker(rank, world_size, cfg):
    ...
    model = MoCo(encoder_q, encoder_k, generator=cfg.generator, **cfg.moco)  # cfg.moco.dim, K, m, T, mlp
    model.cuda()
